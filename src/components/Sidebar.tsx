...
const Sidebar = ({ isOpen, onToggle, onApiKeyChange }: SidebarProps) => {
  ...
  const defaultCategories = [
    { title: "Popular Categories", items: ["Action Movies", "Comedy", "Drama", "Horror", "Sci-Fi"] },
    { 
      title: "My Categories", 
      items: [
  ...
  return (
    <div className={cn(
      "fixed top-0 left-0 z-40 h-screen bg-movie-main transition-all duration-300",
      isOpen ? "w-64" : "w-0"
    )}>
      <nav className="flex h-full w-full flex-col px-3" aria-label="Movie categories">
        <div className="flex justify-between flex h-[60px] items-center">
          <button onClick={onToggle} className="h-10 rounded-lg px-2 text-movie-text hover:bg-movie-main/50">
            <Menu className="h-5 w-5" />
          </button>
          <button className="flex items-center gap-2 rounded-lg px-3 py-1 text-sm hover:bg-movie-main/50">
          ...
          <div className="pt-0">
            {defaultCategories.map((category) => (
              <div key={category.title} className="mb-6">
                <div className="px-3 py-2 text-xs text-movie-muted font-medium">{category.title}</div>
